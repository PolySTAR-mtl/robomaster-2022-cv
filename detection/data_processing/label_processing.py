...
# Directories name
sets=['robomaster_Central China Regional Competition', 'robomaster_Final Tournament', 'robomaster_North China Regional Competition', 'robomaster_South China Regional Competition']
# List of classes
...
def convert_annotation(set_name, image_id):
    in_file = open('../dataset/%s/image_annotation/%s.xml'%(set_name, image_id))
...
if __name__ == "__main__":
  ...
  for set_name in sets:
    ...
    image_ids = [f for f in listdir('../dataset/%s/image/'%(set_name)) if isfile(join('../dataset/%s/image/'%(set_name), f))]
